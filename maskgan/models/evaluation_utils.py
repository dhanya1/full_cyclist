...
def print_and_log_losses(log, step, is_present_rate, avg_dis_loss,
  ...
  print(' D train_bkp loss: %.5f' % np.mean(avg_dis_loss))
  print(' G train_bkp loss: %.5f' % np.mean(avg_gen_loss))
  log.write('\nglobal_step: %d\n' % step)
  log.write((' is_present_rate: %.3f\n' % is_present_rate))
  log.write(' D train_bkp loss: %.5f\n' % np.mean(avg_dis_loss))
  log.write(' G train_bkp loss: %.5f\n' % np.mean(avg_gen_loss))
