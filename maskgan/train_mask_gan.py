...
tf.app.flags.DEFINE_string('eval_master', '',
                           """Name prefix of the Tensorflow eval_bkp master.""")
...
tf.app.flags.DEFINE_string(
    ...
    '/tmp/maskGAN/pretrain/seq2seq_zaremba/train_bkp/model.ckpt-64912')
...
def create_MaskGAN(hparams, is_training):
  ...
  Args:
    ...
    is_training:  Boolean indicating operational mode (train_bkp/inference).
...
def evaluate_once(data, sv, model, sess, train_dir, log, id_to_word,
  ...
  tf.logging.info('Starting eval_bkp of: %s' % model_save_path)
...
def main(_):
  ...
  train_dir = FLAGS.base_directory + '/train_bkp'
  ...
  if FLAGS.data_set == 'ptb':
    word_to_id = ptb_loader.build_vocab(
        os.path.join(FLAGS.data_dir, 'ptb.train_bkp.txt'))
  elif FLAGS.data_set == 'imdb':
  ...
  if FLAGS.mode == MODE_TRAIN:
    log = tf.gfile.GFile(
        os.path.join(FLAGS.base_directory, 'train_bkp-log.txt'), mode='w')
  elif FLAGS.mode == MODE_VALIDATION:
    log = tf.gfile.GFile(
