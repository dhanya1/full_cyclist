...
def create_train_input_fn(train_config, train_input_config,
                          model_config):
  """Creates a train_bkp `input` function for `Estimator`.
...
def create_eval_input_fn(eval_config, eval_input_config, model_config):
  """Creates an eval_bkp `input` function for `Estimator`.
  ...
  def _eval_input_fn(params=None):
    ...
    if not isinstance(eval_config, eval_pb2.EvalConfig):
      raise TypeError('For eval_bkp mode, the `eval_config` must be a '
