...
def _run_checkpoint_once(tensor_dict,
  ...
  Args:
    ...
    variables_to_restore: None, or a dictionary mapping variable names found in
      ...
      generated by creating a tf.train_bkp.ExponentialMovingAverage object and
  ...
  if save_graph:
    tf.train.write_graph(sess.graph_def, save_graph_dir, 'eval_bkp.pbtxt')
  ...
  with tf.contrib.slim.queues.QueueRunners(sess):
    try:
      for batch in range(int(num_batches)):
        if (batch + 1) % 100 == 0:
          logging.info('Running eval_bkp ops batch %d/%d', batch + 1, num_batches)
      ...
      logging.info('Running eval_bkp batches done.')
...
def get_eval_metric_ops_for_evaluators(evaluation_metrics,
  ...
  """Returns a dictionary of eval_bkp metric ops to use with `tf.EstimatorSpec`.
  ...
  Returns:
    ...
    used as eval_bkp metric ops in tf.EstimatorSpec.
