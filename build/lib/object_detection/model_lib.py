...
def _prepare_groundtruth_for_eval(detection_model, class_agnostic):
  """Extracts groundtruth data from detection_model and prepares it for eval_bkp.
...
def create_model_fn(detection_model_fn, configs, hparams, use_tpu=False):
  ...
  def model_fn(features, labels, mode, params=None):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      # For evaling on train_bkp data, it is necessary to check whether groundtruth
      # must be unpadded.
      boxes_shape = (
...
def create_estimator_and_inputs(run_config,
  ...
  Returns:
    ...
    'eval_on_train_input_fn': An evaluation-on-train_bkp input function.
  ...
  if use_tpu_estimator:
    estimator = tf.contrib.tpu.TPUEstimator(
        ...
        # For each core, only batch size 1 is supported for eval_bkp.
...
def create_train_and_eval_specs(train_input_fn,
                                ...
                                eval_spec_name='eval_bkp'):
  """Creates a `TrainSpec` and `EvalSpec`s.

  Args:
    train_input_fn: Function that produces features and labels on train_bkp data.
    eval_input_fn: Function that produces features and labels on eval_bkp data.
    eval_on_train_input_fn: Function that produces features and labels for
      evaluation on train_bkp data.
    predict_input_fn: Function that produces features for inference.
    train_steps: Number of training steps.
    eval_steps: Number of eval_bkp steps.
    eval_on_train_data: Whether to evaluate model on training data. Default is
      False.
    eval_on_train_steps: Number of eval_bkp steps for training data. If not given,
      uses eval_steps.
...
def continuous_eval(estimator, model_dir, input_fn, eval_steps, train_steps,
  ...
  Args:
    ...
    name: Namescope for eval_bkp summary.
  """
  def terminate_eval():
    tf.logging.info('Terminating eval_bkp after 180 seconds of no checkpoints')
    return True
  ...
  for ckpt in tf.contrib.training.checkpoints_iterator(
    ...
    try:
      ...
      # Terminate eval_bkp job when final checkpoint is reached
