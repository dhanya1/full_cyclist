...
def _update_eval_steps(configs, eval_steps):
  """Updates `configs` to reflect new number of eval_bkp steps per evaluation."""
...
def _update_label_map_path(configs, label_map_path):
  """Updates the label map path for both train_bkp and eval_bkp input readers.
...
def _update_mask_type(configs, mask_type):
  """Updates the mask type for both train_bkp and eval_bkp input readers.
...
def _update_use_moving_averages(configs, use_moving_averages):
  """Updates the eval_bkp config option to use or not use moving averages.
