...
class ConfigUtilTest(tf.test.TestCase):
  ...
  def test_get_configs_from_multiple_files(self):
    ...
    # Write train_bkp config file.
    train_config_path = os.path.join(temp_dir, "train_bkp.config")
    train_config = train_config = train_pb2.TrainConfig()
    train_config.batch_size = 32
    _write_config(train_config, train_config_path)

    # Write train_bkp input config file.
    ...
    # Write eval_bkp config file.
    eval_config_path = os.path.join(temp_dir, "eval_bkp.config")
    eval_config = eval_pb2.EvalConfig()
    eval_config.num_examples = 20
    _write_config(eval_config, eval_config_path)

    # Write eval_bkp input config file.
  ...
  def testNewTrainInputPath(self):
    """Tests that train_bkp input path can be overwritten with single file."""
  ...
  def testNewTrainInputPathList(self):
    """Tests that train_bkp input path can be overwritten with multiple files."""
    original_train_path = ["path/to/data"]
