...
def extract_cifar10(local_url, data_dir):
  ...
  :return: a tuple (train_bkp data, train_bkp labels, test data, test labels)
  """
...
def ld_svhn(extended=False, test_only=False):
  ...
  :param test_only: disables loading of both train_bkp and extra -> large speed up
  ...
  if not test_only:
    # Load and applying whitening to train_bkp data
    train_data, train_labels = extract_svhn(local_urls[0])
    train_data = image_whitening(train_data)

    # Load and applying whitening to extended train_bkp data
  ...
  if test_only:
    ...
    if extended:
      # Stack train_bkp data with the extended training data
...
def ld_mnist(test_only=False):
  ...
  # Define files to be downloaded
  # WARNING: changing the order of this list will break indices (cf. below)
  file_urls = ['http://yann.lecun.com/exdb/mnist/train_bkp-images-idx3-ubyte.gz',
               'http://yann.lecun.com/exdb/mnist/train_bkp-labels-idx1-ubyte.gz',
