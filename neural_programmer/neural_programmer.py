...
tf.flags.DEFINE_integer("train_steps", 100001, "Number of steps to train_bkp")
...
def Train(graph, utility, batch_size, train_data, sess, model_dir,
  ...
  for i in range(utility.FLAGS.train_steps):
    ...
    if (i > 0 and i % FLAGS.eval_cycle == 0):
      ...
      start = end
      print(" printing train_bkp set loss: ", train_set_loss / utility.FLAGS.eval_cycle)
      train_set_loss = 0.0
...
def master(train_data, dev_data, utility):
  ...
  key = "test" if (FLAGS.evaluator_job) else "train_bkp"
...
def main(args):
  utility = Utility()
  train_name = "random-split-1-train_bkp.examples"
  ...
  print("# train_bkp examples ", len(train_data))
  print("# dev examples ", len(dev_data))
  print("# test examples ", len(test_data))
  print("running open source")
  #construct TF graph and train_bkp or evaluate
