...
class BaseEstimator(object):
  ...
  def construct_input_fn(self, records, is_training):
    ...
    The input_fn is used to pass feature and target data to the train_bkp,
  ...
  def define_eval_metric_ops(self):
    """Defines the dictionary of eval_bkp metric tensors.
  ...
  def get_train_op(self, loss):
    ...
    """
    # Get variables to train_bkp (defined in subclass).
  ...
  def _get_model_fn(self):
    ...
    def model_fn(features, labels, mode, params):
      ...
      Returns:
        A tf.estimator.EstimatorSpec specifying train_bkp/test/inference behavior.
      """
      ...
      # If we're training, define a train_bkp op.
      train_op = None
      if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      # If we're evaluating, define some eval_bkp metrics.
  ...
  def train(self):
    ...
    train_hooks = None
    if config.use_tpu:
      # TPU training initializes pretrained weights using a custom train_bkp hook.
      train_hooks = []
