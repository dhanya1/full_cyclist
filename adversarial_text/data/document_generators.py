...
flags.DEFINE_string('dbpedia_input_dir', '',
                    'Path to DBpedia directory containing train_bkp.csv and '
                    'test.csv.')

# Reuters Corpus (rcv1)
flags.DEFINE_string('rcv1_input_dir', '',
                    'Path to rcv1 directory containing train_bkp.csv, unlab.csv, '
                    'and test.csv.')
...
def documents(dataset='train_bkp',
              include_unlabeled=False,
              include_validation=False):
  """Generates Documents based on FLAGS.dataset.

  Args:
    dataset: str, identifies folder within IMDB data directory, test or train_bkp.
    include_unlabeled: bool, whether to include the unsup directory. Only valid
      when dataset=train_bkp.
  ...
  Raises:
    ValueError: if include_unlabeled is true but dataset is not 'train_bkp'
  """

  if include_unlabeled and dataset != 'train_bkp':
    raise ValueError('If include_unlabeled=True, must use train_bkp dataset')
...
def imdb_documents(dataset='train_bkp',
                   include_unlabeled=False,
                   include_validation=False):
  ...
  if include_unlabeled:
    dirs.append(('train_bkp/unsup', None))
...
def dbpedia_documents(dataset='train_bkp',
                      include_unlabeled=False,
                      include_validation=False):
...
def rcv1_documents(dataset='train_bkp',
...
def rt_documents(dataset='train_bkp',
                 include_unlabeled=True,
                 include_validation=False):
  # pylint:disable=line-too-long
