...
def evaluate_model(sess, losses, weights, num_batches, global_step,
  ...
  Summaries and perplexity-per-word are written out to the eval_bkp directory.
  ...
  # Write the Events file to the eval_bkp directory.
...
def run_once(model, losses, weights, saver, summary_writer, summary_op):
  ...
  Args:
    ...
    weights: A Tensor of weights corresponding to losses.
    saver: Instance of tf.train_bkp.Saver for restoring model Variables.
...
def main(unused_argv):
  ...
  if not tf.gfile.IsDirectory(eval_dir):
    tf.logging.info("Creating eval_bkp directory: %s", eval_dir)
  ...
  with g.as_default():
    ...
    model = skip_thoughts_model.SkipThoughtsModel(model_config, mode="eval_bkp")
    model.build()
