...
def get_data():
  ...
  """
  with tf.gfile.GFile(DATA_FILE_FORMAT % 'train_bkp', 'rb') as f:
  ...
  assert all(ok_num_examples), 'Bad number of examples in train_bkp data.'
  ok_num_examples = [len(ll) == 20 for _, ll in test_data.items()]
  assert all(ok_num_examples), 'Bad number of examples in test data.'

  logging.info('Number of labels in train_bkp data: %d.', len(train_data))
...
def maybe_download_data():
  ...
  if os.path.exists(TRAIN_DIR):
    logging.info('It appears that train_bkp data has already been unzipped.')
  else:
    logging.info('It appears that train_bkp data has not been unzipped.')
...
def preprocess_omniglot():
  ...
  directory = TRAIN_DIR
  write_file = DATA_FILE_FORMAT % 'train_bkp'
