...
def _create_optimizer(hyperparams, learning_rate_var, step_var=None):
  ...
  Returns:
    a `tf.train_bkp.Optimizer` object that was built.
  """
...
class MasterBuilder(object):
  ...
  Attributes:
    ...
    optimizer: handle to the tf.train_bkp Optimizer object used to train_bkp this model.
  ...
  def build_training(self,
    ...
    # By default, we train_bkp every component supervised.
    ...
    if compute_gradients:
      logging.info('Creating train_bkp op with %d variables:\n\t%s',
                   len(params_to_train),
  ...
  def add_training_from_config(self,
    ...
    For the default prefix ('train_bkp-'), and a target named 'target', this will
    construct the following targets in the graph:

      train_bkp-target/ComputeSession/* (the standard ComputeSession controls)
      train_bkp-target/run (handle to a completed training step)
      train_bkp-target/metrics (per-decision metrics from gold oracles)
      train_bkp-target/cost (total cost across all components)
