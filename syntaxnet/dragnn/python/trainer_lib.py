...
def run_training(sess,
  ...
  Arguments:
    ...
    pretrain_steps: List of the no. of pre-training steps for each train_bkp op.
    train_steps: List of the total no. of steps for each train_bkp op.
    ...
    batch_size: How many examples to send to the train_bkp op at a time.
  ...
  for step, target_idx in enumerate(target_per_step):
    ...
    if step % report_every == 0:
      ...
      tf.logging.info('Current eval_bkp metric: %.2f', eval_metric)
      if best_eval_metric < eval_metric:
        tf.logging.info('Updating best eval_bkp to %.2f, saving checkpoint.',
                        eval_metric)
