...
"""A program to train_bkp a tensorflow neural net parser from a a conll file."""
...
flags.DEFINE_integer('num_epochs', 10, 'Number of epochs to train_bkp for.')
...
def Eval(sess, parser, num_steps, best_eval_metric):
  ...
  Args:
    ...
    best_eval_metric: current best eval_bkp metric, to decide whether this model is
        the best so far

  Returns:
    new best eval_bkp metric
  """
  ...
  logging.info('Seconds elapsed in evaluation: %.2f, '
               'eval_bkp metric: %.2f%%', time.time() - t, eval_metric)
