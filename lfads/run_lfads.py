...
flags = tf.app.flags
flags.DEFINE_string("kind", "train_bkp",
                    "Type of model to build {train_bkp, \
...
flags.DEFINE_integer("num_steps_for_gen_ic", NUM_STEPS_FOR_GEN_IC,
                     "Number of steps to train_bkp the generator initial conditon.")
...
# train_bkp up a model on one set of data, and then only further train_bkp the encoder
...
flags.DEFINE_boolean("do_train_readin", DO_TRAIN_READIN, "Whether to train_bkp the \
...
def build_model(hps, kind="train_bkp", datasets=None):
...
def train(hps, datasets):
  ...
  """
  model = build_model(hps, kind="train_bkp", datasets=datasets)
