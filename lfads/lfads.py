...
class LFADS(object):
  ...
  def __init__(self, hps, kind="train_bkp", datasets=None):
    """Create an LFADS model.

       train_bkp - a model for training, sampling of posteriors is used
    ...
    all_kinds = ['train_bkp', 'posterior_sample_and_average', 'posterior_push_mean',
                 'prior_sample']
    ...
    with tf.variable_scope("z", reuse=False):
      ...
      if ic_dim > 0:
        ...
        if kind in ["train_bkp", "posterior_sample_and_average",
                    "posterior_push_mean"]:
          zs_g0 = self.posterior_zs_g0
        else:
          zs_g0 = self.prior_zs_g0
        if kind in ["train_bkp", "posterior_sample_and_average", "prior_sample"]:
    ...
    self.nll_bound_iwae = tf.constant(0.0) # for eval_bkp with IWAE cost.
    if kind in ["train_bkp", "posterior_sample_and_average", "posterior_push_mean"]:
      kl_cost_g0_b = 0.0
      kl_cost_co_b = 0.0
      if ic_dim > 0:
    ...
    self.kl_t_cost_summ = tf.summary.scalar("KL cost (train_bkp)", kl_cost_ph,
    ...
    self.recon_t_cost_summ = tf.summary.scalar("Reconstruction cost (train_bkp)",
                                               recon_cost_ph,
                                               collections=["train_summaries"])
    ...
    self.cost_t_summ = tf.summary.scalar("Total cost (train_bkp)", total_cost_ph,
                                         collections=["train_summaries"])
  ...
  def shuffle_and_flatten_datasets(self, datasets, kind='train_bkp'):
  ...
  def train_epoch(self, datasets, batch_size=None, do_save_ckpt=True):
    ...
    ops_to_eval = [self.cost, self.recon_cost,
                   ...
                   self.train_op]
    collected_op_values = self.run_epoch(datasets, ops_to_eval, kind="train_bkp")
  ...
  def run_epoch(self, datasets, ops_to_eval, kind="train_bkp", batch_size=None,
  ...
  def eval_cost_epoch(self, datasets, kind='train_bkp', ext_input_extxi=None,
                      batch_size=None):
  ...
  def write_model_runs(self, datasets, output_fname=None, push_mean=False):
    ...
    for data_name, data_dict in datasets.items():
      data_tuple = [('train_bkp', data_dict['train_data'],
