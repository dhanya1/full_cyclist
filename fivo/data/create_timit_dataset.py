...
tf.app.flags.DEFINE_float("valid_frac", 0.05,
                          "Fraction of train_bkp set to use as valid set. "
...
def main(unused_argv):
  ...
  print("%d train_bkp / %d valid / %d test" % (
  ...
  # Make sure there is no overlap in the train_bkp, test, and valid sets.
  ...
  # Calculate the mean and standard deviation of the train_bkp set.
  train_stacked = np.hstack(train_wavs)
  train_mean = np.mean(train_stacked)
  train_std = np.std(train_stacked)
  print("train_bkp mean: %f  train_bkp std: %f" % (train_mean, train_std))

  # Process all data, normalizing with the train_bkp set statistics.
  ...
  create_tfrecord_from_wavs(
      ...
      os.path.join(FLAGS.out_dir, "train_bkp"))
  create_tfrecord_from_wavs(
