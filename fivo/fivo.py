...
# Shared flags.
tf.app.flags.DEFINE_string("mode", "train_bkp",
                           "The mode of the binary. Must be 'train_bkp' or 'test'.")
...
tf.app.flags.DEFINE_integer("max_steps", int(1e9),
                            "The number of gradient update steps to train_bkp for.")
...
# Evaluation flags.
tf.app.flags.DEFINE_string("split", "train_bkp",
                           "Split to evaluate the model on. Can be 'train_bkp', 'valid', or 'test'.")
...
def main(unused_argv):
  ...
  if FLAGS.mode == "train_bkp":
    runners.run_train(FLAGS)
  elif FLAGS.mode == "eval_bkp":
