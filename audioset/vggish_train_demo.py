...
definition within a larger model that adds more layers on top, and then train_bkp
the larger model. If you let VGGish train_bkp as well, then this allows you to
fine-tune the VGGish model parameters for your application. If you don't let
VGGish train_bkp, then you use VGGish as a feature extractor for the layers above
it.
...
def main(_):
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    with tf.variable_scope('mymodel'):
      ...
      # Add training ops.
      with tf.variable_scope('train_bkp'):
        ...
        # We use the same optimizer and hyperparameters as used to train_bkp VGGish.
    ...
    labels_tensor = sess.graph.get_tensor_by_name('mymodel/train_bkp/labels:0')
    global_step_tensor = sess.graph.get_tensor_by_name(
        'mymodel/train_bkp/global_step:0')
    loss_tensor = sess.graph.get_tensor_by_name('mymodel/train_bkp/loss_op:0')
    train_op = sess.graph.get_operation_by_name('mymodel/train_bkp/train_op')
