...
flags.DEFINE_string('source_split_name', 'train_bkp',
                    'Name of the train_bkp split for the source.')

flags.DEFINE_string('target_split_name', 'train_bkp',
                    'Name of the train_bkp split for the target.')
...
def _train(discriminator_train_op,
  ...
  Args:
    ...
    scaffold: An tf.train_bkp.Scaffold instance.
    hooks: List of `tf.train_bkp.SessionRunHook` callbacks which are run inside the
      training loop.
    chief_only_hooks: List of `tf.train_bkp.SessionRunHook` instances which are run
