...
can be used to train_bkp and evaluate models that classify Kepler TCEs.
...
The output TFRecord file contains one serialized tensorflow.train_bkp.Example
...
def _set_float_feature(ex, name, value):
  """Sets the value of a float feature in a tensorflow.train_bkp.Example proto."""
...
def _set_bytes_feature(ex, name, value):
  """Sets the value of a bytes feature in a tensorflow.train_bkp.Example proto."""
  assert name not in ex.features.feature, "Duplicate feature: %s" % name
...
def _set_int64_feature(ex, name, value):
  """Sets the value of an int64 feature in a tensorflow.train_bkp.Example proto."""
  assert name not in ex.features.feature, "Duplicate feature: %s" % name
...
def _process_tce(tce):
  ...
  Returns:
    A tensorflow.train_bkp.Example proto containing TCE features.
...
def main(argv):
  ...
  for i in range(FLAGS.num_train_shards):
    ...
    file_shards.append((train_tces[start:end], os.path.join(
        FLAGS.output_dir, "train_bkp-%.5d-of-%.5d" % (i, FLAGS.num_train_shards))))
