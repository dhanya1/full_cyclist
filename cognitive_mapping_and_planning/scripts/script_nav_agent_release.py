...
r""" Script to train_bkp and test the grid navigation agent.
Usage:
  1. Testing a model.
    ...
    with tf.device(tf.train.replica_device_setter(args.solver.ps_tasks,
      ...
      with tf.container(container_name):
        m = args.setup_to_run(m, args, is_training=True,
                             batch_norm_is_training=True, summary_mode='train_bkp')

        train_step_kwargs = args.setup_train_step_kwargs(
            m, R(), os.path.join(args.logdir, 'train_bkp'), rng_seed=args.solver.task,
