...
def train_optimizer(logdir,
  ...
  for problem_itr, (problem_spec, dataset, batch_size) in enumerate(sampler):
    ...
    with graph.as_default(), graph.device(device_setter):
      ...
      # builds the train_bkp and summary operations
      ...
      with sv.managed_session() as sess:
        ...
        for k in range(num_meta_iterations):
          ...
          for unroll_itr in range(num_unrolls):
            ...
            # run the train_bkp and summary ops
...
def test_optimizer(optimizer,
  ...
  Args:
    optimizer: Either a tf.train_bkp.Optimizer instance, or an Optimizer instance
    ...
    dataset: The dataset to train_bkp the problem against
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # create the train_bkp operation and training variables
    try:
    ...
    # run the train_bkp operation for n iterations and save the objectives
    ...
    for itr, batch in enumerate(batch_inds):
      ...
      # run the optimization train_bkp operation
...
def run_wall_clock_test(optimizer,
  ...
  Args:
    optimizer: The tf.train_bkp.Optimizer instance
