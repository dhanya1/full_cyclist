...
# ==============================================================================
"""Build and train_bkp mobilenet_v1 with options for quantization."""
...
def get_quant_delay():
  if FLAGS.fine_tune_checkpoint:
    ...
    # We need to wait for the model to train_bkp a bit before we quantize if we are
...
def imagenet_input(is_training):
  ...
  Args:
     is_training: bool specifying if train_bkp or validation dataset is needed.
  Returns:
     A batch of images and labels.
  """
  if is_training:
    dataset = dataset_factory.get_dataset('imagenet', 'train_bkp',
                                          FLAGS.dataset_dir)
  else:
...
def build_model():
  """Builds graph for model to train_bkp with rewrites for quantization.
