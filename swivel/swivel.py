...
flags.DEFINE_float('num_epochs', 40, 'Number epochs to train_bkp')
...
class Model(object):
  ...
  def wordsim_eval_op(self, filename):
    """Returns an op that runs an eval_bkp on a word similarity dataset.

    The eval_bkp dataset is assumed to be tab-separated, one scored word pair per
  ...
  def analogy_eval_op(self, filename, max_vocab_size=20000):
    """Returns an op that runs an eval_bkp on an analogy dataset.

    The eval_bkp dataset is assumed to be tab-separated, with four tokens per
    ...
    # Take the four nearest neighbors, since the eval_bkp explicitly discards the
    # query terms.
...
def main(_):
  ...
  with tf.Graph().as_default():
    with tf.device(device_setter):
      ...
      # If an eval_bkp path is present, then create eval_bkp operators and set up scalar
      # summaries to report on the results.  Run the evals on the CPU since
      # the analogy eval_bkp requires a fairly enormous tensor to be allocated to
