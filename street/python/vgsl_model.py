...
def Train(train_dir,
  ...
  with tf.Graph().as_default():
    with tf.device(device):
      model = InitNetwork(train_data, model_str, 'train_bkp', initial_learning_rate,
...
def Eval(train_dir,
  ...
  Args:
    ...
    num_steps: Number of eval_bkp steps to run.
  ...
  rates = ec.ErrorRates(
      ...
      sequence_error=None)
  with tf.Graph().as_default():
    model = InitNetwork(eval_data, model_str, 'eval_bkp', reader=reader)
    sw = tf.summary.FileWriter(eval_dir)

    while True:
      sess = tf.Session('')
      if graph_def_file is not None:
        # Write the eval_bkp version of the graph to a file for freezing.
...
def InitNetwork(input_pattern,
  ...
  Args:
    ...
    mode: One of 'train_bkp', 'eval_bkp'
...
class VGSLImageModel(object):
  ...
  def TrainAStep(self, sess):
    ...
    Args:
      sess: Session in which to train_bkp the model.
    Returns:
      loss, global_step.
    """
  ...
  def RunAStep(self, sess):
    """Runs a step for eval_bkp in the session.
  ...
  def _AddOutputs(self, prev_layer, out_dims, out_func, num_classes):
    ...
    if self.mode == 'train_bkp':
      ...
      # Be sure the labels match the output, even in eval_bkp mode.
  ...
  def _AddOptimizer(self, optimizer_type):
    ...
    self.train_op = opt.minimize(
        self.loss, global_step=self.global_step, name='train_bkp')
